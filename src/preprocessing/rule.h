               ...
               DegreeNodeSetVector& degreeVector)
  {
    ...
    for (NodeIt v(g); v != lemon::INVALID; ++v)
    {
      for (IncEdgeIt e(g, v); e != lemon::INVALID; ++e)
      {
        ...
        for (NodeSetIt nodeIt = nodes.begin(); nodeIt != nodes.end(); ++nodeIt)
        {
          for (IncEdgeIt e(g, *nodeIt); e != lemon::INVALID; ++e)
          {
            Node adjNode = g.oppositeNode(*nodeIt, e);
            int d = degree[adjNode]--;
            degreeVector[d].erase(adjNode);
            degreeVector[d-1].insert(adjNode);
            neighbors[adjNode].erase(*nodeIt);
            
            nEdges--;
            nArcs -= 2;
          }
        }
        // remove the nodes from degree vector
        for (NodeSetIt nodeIt = nodes.begin(); nodeIt != nodes.end(); ++nodeIt)
          degreeVector[degree[*nodeIt]].erase(*nodeIt);
        
        // update mapToPre
        for (NodeSetIt nodeIt = nodes.begin(); nodeIt != nodes.end(); ++nodeIt)
        {
          const NodeSet& pre_nodes = preOrigNodes[*nodeIt];
          for (NodeSetIt pre_nodeIt = pre_nodes.begin(); pre_nodeIt != pre_nodes.end(); pre_nodeIt++)
          {
            // unmap
            mapToPre[*pre_nodeIt].erase(*nodeIt);
          }
        }
